"""Block-bootstrap confidence intervals on the naive uncapped full-Kelly
config (max_pos=100%, agg=100%, cat=100%, fraction=1.0) -- the "Kelly
portfolio" route chosen for its higher raw return over the capped config.

Same moving block bootstrap as bootstrap_kelly_ci.py (resampling calendar-
month blocks of trades with replacement, preserving each block's internal
chronology), re-run against the uncapped sizing instead. This matters more
here than for the capped config: with no per-trade ceiling, a single flip
landing on a large position at high equity can now swing the outcome by
itself (the realized path's one loss -- "Will Kamala Harris go on SNL?",
Nov 2024 -- was 33% of bankroll in one trade), so which flip the resample
happens to include, and how much equity had compounded by the time it
lands, matters far more to the distribution than it did under the capped
sizing.
"""
import json
import os
...
UNCAPPED_KW = dict(max_pos_pct=1.0, agg_cap_pct=1.0, cat_cap_pct=1.0)
...
def main():
    ...
    point = run_sim([dict(r) for r in maker_trades], fraction=1.0, **UNCAPPED_KW)
    print(f"\nPoint estimate (actual historical path, uncapped): final=${point['final_equity']:,.0f}  "
    ...
    n_ruin = 0  # final equity < 50% of starting bankroll, a proxy for "this would have hurt badly"
    failures = 0
    for i in range(N_BOOT):
        synthetic = build_synthetic_timeline(blocks, ordered_keys, rng)
        res = run_sim(synthetic, fraction=1.0, **UNCAPPED_KW)
        if res is None:
            failures += 1
            continue
    ...
    def pctiles(xs):
        ...
        return {"p5": p(0.05), "p25": p(0.25), "median": p(0.50), "p75": p(0.75), "p95": p(0.95),
                "min": xs[0], "max": xs[-1], "mean": sum(xs) / n, "n": n}
    ...
    results = {
        ...
        "pct_boot_runs_below_half_bankroll": n_ruin / len(boot_final) * 100,
    }

    print(f"\n=== Bootstrap distribution ({N_BOOT} resamples, {len(ordered_keys)}-month blocks, UNCAPPED) ===")
    for metric in ["final_equity", "cagr_pct", "max_drawdown_pct", "sharpe"]:
        ...
        print(f"{metric:<18} p5={p['p5']:>10.2f}  p25={p['p25']:>10.2f}  median={p['median']:>10.2f}  "
              f"p75={p['p75']:>10.2f}  p95={p['p95']:>10.2f}  min={p['min']:>10.2f}  max={p['max']:>10.2f}")
    print(f"\nFraction of bootstrap resamples that lost money: {results['pct_boot_runs_losing_money']:.1f}%")
    print(f"Fraction of bootstrap resamples that fell below 50% of starting bankroll: "
          f"{results['pct_boot_runs_below_half_bankroll']:.1f}%")
    print(f"n_flips_taken distribution: {results['n_flips_taken']}")

    out_path = os.path.join(DATA_DIR, "kelly_bootstrap_ci_uncapped.json")
